...
async def reply_customer_message(bot: Bot, event: PrivateMessageEvent):
    ...
    if ticket_id is None: # 如果没有工单
        ...
        # 创建工单
        ticket_id = create_ticket(uid, datetime.fromtimestamp(event.time / 1000 - 1))
    if get_ticket(ticket_id)['status'] == 'creating': # 如果工单状态为创建中
        update_ticket(ticket_id, creating_expired_at=datetime.fromtimestamp(event.time / 1000 + 120)) # 更新工单创建中过期时间
        # 等待120秒
        await sleep(120)
        # 如果当前时间大于工单创建中过期时间
        if datetime.now() > get_ticket(ticket_id)['creating_expired_at']:
            ...
            print_ticket(event=event, bot=bot, ticket_id=ticket_id, target_group_id=config.NOTIFY_GROUP)
        ...
        if get_ticket(ticket_id)['engineer_id'] == '':
            update_ticket(ticket_id, processing_expired_at=datetime.fromtimestamp(event.time / 1000 + 30)) # 更新工单处理中过期时间
            # 等待30秒
            await sleep(30)
            # 如果当前时间大于工单处理中过期时间
            if datetime.now() > get_ticket(ticket_id)['processing_expired_at']:
                # 将工单创建起至当下的消息合并转发至【通知群】
                print_ticket(event=event, bot=bot, ticket_id=ticket_id, target_group_id=config.NOTIFY_GROUP)
                bot.send_group_msg(group_id=config.NOTIFY_GROUP, message="工程师接单超时，请尽快处理！")
