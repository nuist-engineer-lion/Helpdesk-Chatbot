...
async def reply_customer_message(bot: Bot, event: PrivateMessageEvent,sess: async_scoped_session):
    ...
    if ticket_id is None: # 如果没有工单
        ...
        # 创建工单
        ticket_id = await create_ticket(uid, datetime.fromtimestamp(event.time, cst),sess)
        ticket = await get_ticket(ticket_id,sess)
    if ticket['status'] == 'creating': # 如果工单状态为创建中
        await update_ticket(ticket_id,sess, creating_expired_at=datetime.now() + timedelta(seconds=config.waiting_time))
        # 等待waiting_time秒
        await sleep(config.waiting_time+2)
        # 如果当前时间大于工单创建中过期时间
        if datetime.now() > ticket['creating_expired_at'] and ticket['status'] == 'creating':
            await update_ticket(ticket_id, sess, status='processing') # 更新工单状态为处理中
            # 将工单创建起至当下的消息合并转发至【通知群】
            await customer_message.send("您的问题已经记录并提交工程师，正在派单请稍等...")
            await print_ticket(event=event, bot=bot, ticket_id=ticket_id,sess=sess, target_group_id=config.notify_group, delay=30)
            # @全体成员 有新工单，请尽快处理！
        ...
        if ticket['engineer_id'] == '':
            await update_ticket(ticket_id, sess, processing_expired_at=datetime.now() + timedelta(seconds=config.waiting_time / 2))
            # 等待waiting_time秒
            await sleep(config.waiting_time/2+2)
            # 如果当前时间大于工单处理中过期时间
            if datetime.now() > ticket['processing_expired_at']:
                # 将工单创建起至当下的消息合并转发至【通知群】
            ...
            # 将消息转发至工程师
            await bot.send_private_msg(user_id=ticket['engineer_id'], message=event.get_message())

# 回复工程师消息
@engineer_message.handle()
async def reply_engineer_message(bot: Bot, event: MessageEvent,sess: async_scoped_session):
    uid = event.get_user_id()
    ...
    if "查看工单" in plain_message:
        ...
        for ticket_id in ticket_ids:
            if ['status'] != 'closed':
                await print_ticket(event=event, bot=bot, ticket_id=ticket_id,sess=sess)
    elif "取消接单" in plain_message:
        if ticket_id is not None:
            if ticket['status'] == 'processing' and ticket['engineer_id'] == uid:
                await update_ticket(ticket_id,sess, engineer_id='')
        ...
        if ticket['status'] == 'processing' and ticket['engineer_id'] == '':
            await update_ticket(ticket_id,sess, engineer_id=uid)
        ...
        if ticket_id is not None:
            if ticket['status'] == 'processing' and ticket['engineer_id'] == uid:
                await update_ticket(ticket_id,sess, status='closed',end_at=datetime.fromtimestamp(event.time, cst))
        ...
        if ticket_id is not None:
            # 将消息转发至客户
            await bot.send_private_msg(user_id=ticket['customer_id'], message=event.get_message())
